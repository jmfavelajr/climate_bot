...
export async function manageOpenTrades({ flatten = false } = {}) {
  ...
  let rows = mergeRows(dbRows, positions);
  ...
  for (const row of rows) {
    ...
    const opened = row.run_at || row.opened_at || 'unknown';
    ...
    if (liveFav && row.market_ticker === liveFav && role === 'runner') {
      ...
      await updateCandidate(row.id, { reason });
      row.reason = reason;
      ...
      console.log(`DEMOTE ${row.market_ticker} favorite -> runner; live fav=${liveFav} (opened ${opened})`);
      await updateCandidate(row.id, { reason });
      row.reason = reason;
    }

    if (Number.isFinite(bid) && row.id) {
    ...
    if (row.from_live && !row.id) {
      await persistCandidate(
        ...
        null,
        { action: 'live', reason, entry_yes_ask: entry, opened_at: new Date().toISOString() }
      );
    }
    ...
    const decision = exitDecision({ reason, entry, bid, flatten });
    const count = positionCount(positions, row.market_ticker);
    if (!decision.sell) {
      console.log(
        `Manage hold ${row.market_ticker} ${reason} opened=${opened} entry=${entry} bid=${bid} sl=${decision.sl} liveFav=${liveFav} (${decision.why})`
      );
      continue;
    }
    if (count <= 0) {
    ...
    try {
      console.log(`Selling YES ${row.market_ticker} count=${count} @ ${bid} (${decision.why}) opened=${opened}`);
